...
const cacheFile = path.join(process.cwd(), '.bankCache.json'); // File lưu cache
...
async function getCachedBankList() {
  try {
    ...
    console.log('Bank list cache not found or expired.');
  }
  ...
  if (data.code === '00' && data.data) {
    // Lưu vào cache
    const now = Math.floor(Date.now() / 1000);
    const expiry = now + cacheTTL;
    fs.writeFileSync(
      cacheFile,
      JSON.stringify({ data: data.data, expiry: expiry })
    );
...
export { query, hashPassword, initDatabase, getCachedBankList };
