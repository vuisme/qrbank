...
const pool = new Pool({
  ...
  ssl: {
    rejectUnauthorized: false, // Chúờng development
  },
});
...
export async function query({ query, values = [] }) {
  try {
    ...
    console.log("Executing query:", query, values); // Thêm log
    const result = await client.query(query, values);
    console.log("Query result:", result); // Thêm log
    client.release();
    return result.rows; // Sửa lại thành return result.rows thay vì result
  } catch (error) {
    console.error(error);
    throw error;
  }
...
// Gọi hàm initDatabase khi ứng dụng khởi động
initDatabase();